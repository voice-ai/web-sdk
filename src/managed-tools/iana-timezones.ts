...
function parseOffsetMinutes(rawOffset: string): number {
...
function getTimezoneOffsetMinutes(timezoneId: string, date: Date = new Date()): number {
  try {
...
export function formatIanaTimezoneLabel(timezoneId: string, date: Date = new Date()): string {
  return `${formatUtcOffset(getTimezoneOffsetMinutes(timezoneId, date))} • ${timezoneId}`;
}

export const IANA_TIMEZONE_OPTIONS: ReadonlyArray<IanaTimezoneOption> = Object.freeze(
  [...IANA_TIMEZONE_IDS]
    .map((timezoneId) => {
